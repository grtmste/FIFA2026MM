...
export default function MatchCard({ match }: { match: Match }) {
  ...
  return (
    <Link
      ...
      className="group flex overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-card transition-all duration-200 hover:-transtone-y-0.5 hover:border-gold hover:shadow-card-hover"
      style={{ borderLeft: `4px solid ${groupColor(match.group_name)}` }}
    >
      <div className="flex w-9 flex-shrink-0 items-center justify-center bg-stone-50 text-xs font-bold text-stone-400">
        {match.id}
      </div>
